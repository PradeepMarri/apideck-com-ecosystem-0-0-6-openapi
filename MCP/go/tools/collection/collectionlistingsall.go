...
func CollectionlistingsallHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/ecosystems/%s/collections/%s/listings%s", cfg.BaseURL, ecosystem_id, id, queryString)
...
func CreateCollectionlistingsallTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_ecosystems_ecosystem_id_collections_id_listings",
		mcp.WithDescription("List collection listings"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CollectionlistingsallHandler(cfg),
	}
}
