...
func ListingsallHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/ecosystems/%s/listings%s", cfg.BaseURL, ecosystem_id, queryString)
...
func CreateListingsallTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_ecosystems_ecosystem_id_listings",
		mcp.WithDescription("List listings"),
		...
		mcp.WithString("external_id", mcp.Description("Filter on external_id")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListingsallHandler(cfg),
	}
}
