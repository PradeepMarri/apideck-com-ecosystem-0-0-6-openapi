...
func ProductlistingsallHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/ecosystems/%s/products/%s/listings%s", cfg.BaseURL, ecosystem_id, id, queryString)
...
func CreateProductlistingsallTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_ecosystems_ecosystem_id_products_id_listings",
		mcp.WithDescription("List product listings"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductlistingsallHandler(cfg),
	}
}
