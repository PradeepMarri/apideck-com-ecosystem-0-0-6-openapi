...
func ProductsoneHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/ecosystems/%s/products/%s", cfg.BaseURL, ecosystem_id, id)
		...
		var result models.GetProductResponse
...
func CreateProductsoneTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_ecosystems_ecosystem_id_products_id",
		mcp.WithDescription("Get product"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProductsoneHandler(cfg),
	}
}
